...
let debug = false;
let chatOpsCommand = "network mtr"; // Set this value for accurate reporting
////////////////////////////////////////////////////////////////////////////////

module.exports = function (controller) {
    controller.hears(['network mtr'], 'direct_message,direct_mention', (bot, message) => {
        ...
        (async function () {
            try {
                ...
                // Reply to the human
                let reply = `Hello ${firstName}, just a moment please while I try to mtr ${target}`;
                bot.reply(message, reply);

                // Prepare to execute a native OS command
                let linMtrCommand = "mtr " + target + " --report --show-ips --aslookup --report-wide --report-cycles=100"; // Use this command on Linux
                let execThisCommand = linMtrCommand;

                // Perform the command via child process exec
                exec(execThisCommand, function (error, stdout, stderr) {
                    if (stderr) {
                        let thisErrMsg = stderr;
                        let reply = `Sorry ${firstName}, network mtr FAILED:\n\`\`\`${thisErrMsg}\`\`\``;
                        bot.reply(message, reply);
                        // Log the interaction
                        message.logLevel = "WARNING";
                        message.personDetails = personDetails;
                    ...
                    let results = stdout;
                    let reply = `${firstName}, here are the *network mtr ${target}* results you requested:\n\`\`\`${results}\`\`\``;
                    bot.reply(message, reply);
                    // Log the interaction
