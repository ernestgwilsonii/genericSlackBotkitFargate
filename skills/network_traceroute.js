...
let debug = false;
let chatOpsCommand = "network traceroute"; // Set this value for accurate reporting
////////////////////////////////////////////////////////////////////////////////

module.exports = function (controller) {
    controller.hears(['network traceroute'], 'direct_message,direct_mention', (bot, message) => {
        ...
        (async function () {
            try {
                ...
                // Reply to the human
                let reply = `Hello ${firstName}, just a moment please while I try to traceroute ${target}`;
                bot.reply(message, reply);

                // Prepare to execute a native OS command
                let linTracerouteCommand = "traceroute " + target; // Use this command on Linux
                let execThisCommand = linTracerouteCommand;

                // Perform the command via child process exec
                exec(execThisCommand, function (error, stdout, stderr) {
                    if (stderr) {
                        let thisErrMsg = stderr;
                        let reply = `Sorry ${firstName}, network traceroute FAILED:\n\`\`\`${thisErrMsg}\`\`\``;
                        bot.reply(message, reply);
                        // Log the interaction
                        message.logLevel = "WARNING";
                        message.personDetails = personDetails;
                    ...
                    let results = stdout;
                    let reply = `${firstName}, here are the *network traceroute ${target}* results you requested:\n\`\`\`${results}\`\`\``;
                    bot.reply(message, reply);
                    // Log the interaction
